...
# Caminhos
endereco_gtfs = f"../../dados/gtfs/{ano_gtfs}/sppo_{ano_gtfs}-{mes_gtfs}-{quinzena_gtfs}Q.zip"
...
def apuracao(linha):
    try:
        ...
        gps_gdf['id_viagem'] = gps_gdf.groupby(['id_veiculo', 'viagem']).transform(
            lambda x: f"{x['id_veiculo'].iloc[0]}-{linha}-{'I' if x['direction_id_inferred'].iloc[0] == 0 else 'V'}-{x['shape_id'].iloc[0]}-{x['timestamp_gps'].min().strftime('%Y%m%d%H%M%S')}"
        )
